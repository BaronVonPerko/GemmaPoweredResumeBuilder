...
**Exception**: Static `value` on radio and checkbox is **allowed and required** — it identifies which option the input represents, not the bound field value.
...
Do NOT do this: type="radio" [formField]="form.name" or type="checkbox" [formField]="form.name".
...
- **`select` binding**: You CAN bind to select. Ensure options have `value` attributes.
...
| **Multi-select array** | select multiple (string[]) | Use checkboxes for array fields |
| **readonly attribute** | readonly | Use `readonly()` rule in schema |
| **min/max attributes** | min max | Use `min()` and `max()` rules in schema |
| **value binding** | value | Do NOT use `[value]` with `[formField]` (static `value` on radio/checkbox is OK) |
